...
// src/utils/hasSubfield.ts
function hasSubfield(info, pathArray) {
  return info.fieldNodes.some(
    (fieldNode) => findSubfieldRecursive(fieldNode, pathArray)
  );
}
function findSubfieldRecursive(node, pathArray) {
  ...
  return node.selectionSet.selections.some((selection) => {
    ...
    return fieldName === currentField && findSubfieldRecursive(selection, remainingPath);
  });
}
...
var buildPluralField = ({
  ...
  const resolver = async (_, args, context, info) => {
...
var getReadonlyStore = ({
  ...
  findMany: async ({
    ...
    return db.wrap({ method: `${tableName}.findMany` }, async () => {
      let query2 = db.withSchema(namespaceInfo.userNamespace).selectFrom(tableName);
      if (withTotalCount) {
        query2 = db.withSchema(namespaceInfo.userNamespace).with("ponder_totalCount", (db2) => {
          let totalCountQuery = db2.selectFrom(tableName);
          if (where) {
            totalCountQuery = totalCountQuery.where(
              (eb) => buildWhereConditions({ eb, where, table, encoding })
            );
          }
          return totalCountQuery.select(
            ({ fn }) => fn.count("id").as("ponder_totalCount")
          );
      ...
      if (after === null && before === null) {
        ...
        const results = await query2.execute();
        const totalCount = results.at(0)?.ponder_totalCount || 0;
      ...
      if (after !== null) {
        ...
        const results = await query2.execute();
        const totalCount = results.at(0)?.ponder_totalCount || 0;
        const records = results.map((record) => decodeRecord({ record, table, encoding })).reverse();
        if (records.length === 0) {
          return {
            items: records,
            pageInfo: {
